...
from adapters.persistence.sqlmodel.repositories import (
    ...
    SQLMagicLinkRepository,
    SQLPatternRepository,
    SQLUserRepository,
)
from domain.entities.user import User as DomainUser
from infrastructure.auth import ALGORITHM, SECRET_KEY, JWTTokenProvider
...
from use_cases.patterns.get_patterns import (
    GetCatalogUseCase,
...
def get_request_magic_link_use_case(
    user_repo=Depends(get_user_repo), magic_link_repo=Depends(get_magic_link_repo)
):
...
def get_list_favorites_use_case(
    favorite_repo=Depends(get_favorite_repo), pattern_repo=Depends(get_pattern_repo)
):
    return ListFavoritesUseCase(favorite_repo, pattern_repo)


def get_remove_favorite_use_case(favorite_repo=Depends(get_favorite_repo)):
...
async def get_current_user(
    token: str = Depends(reusable_oauth2), user_repo=Depends(get_user_repo)
) -> DomainUser:
    try:
        assert SECRET_KEY is not None  # guaranteed by infrastructure.auth init
