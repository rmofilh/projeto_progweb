...
def user_to_domain(orm: ORMUser) -> DomainUser:
    return DomainUser(
        id=orm.id, email=orm.email, created_at=orm.created_at, last_login_at=orm.last_login_at
    )
...
def collection_to_domain(orm: ORMCollection) -> DomainCollection:
    return DomainCollection(
        id=orm.id, title=orm.title, cover_image_path=orm.cover_image_path, created_at=orm.created_at
    )
