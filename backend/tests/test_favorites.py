...
from adapters.persistence.sqlmodel.models import Pattern, User, UserPattern
...
async def test_add_favorite_limit(authenticated_client, session):
    ...
    p_last = Pattern(
        title="Last", image_path="i", thumbnail_path="t", scale_cm_reference=1, difficulty_level=1
    )
    session.add(p_last)
    await session.commit()
