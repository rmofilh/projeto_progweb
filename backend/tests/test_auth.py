...
from adapters.persistence.sqlmodel.models import MagicLink, User
...
async def test_verify_magic_link_success(client, session):
    ...
    token = "valid-token"
    link = MagicLink(
        user_id=user.id, token=token, expires_at=datetime.utcnow() + timedelta(minutes=15)
    )
    session.add(link)
    await session.commit()
...
async def test_verify_magic_link_expired(client, session):
    ...
    token = "old-token"
    link = MagicLink(
        user_id=user.id, token=token, expires_at=datetime.utcnow() - timedelta(minutes=1)
    )
    session.add(link)
    await session.commit()
